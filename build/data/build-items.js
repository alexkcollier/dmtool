...
class ItemData {
  ...
  async fetchItems(itemPath) {
    const files = await fs.readdir(path.resolve(__dirname, itemPath))
    ...
    return files.reduce(async (asyncData, file) => {
      ...
      try {
        ...
        const items = await fs.readJSON(path.resolve(__dirname, itemPath, file))
        let output = []

        if (items.item) {
          output = JSON.parse(JSON.stringify(items.item))
        }

        if (items.variant) {
          output = JSON.parse(JSON.stringify(this.transformVariants(items.variant)))
        }

        return data.concat(this.parseItemMeta(output))
      } catch (e) {
  ...
  async init() {
    try {
      const data = await this.fetchItems(this.itemPath)
...
exports.build = async function build() {
  try {
    const Official = new ItemData('../../static/data/magic-items')
    const Custom = new ItemData('../../static/data/magic-items/custom')
